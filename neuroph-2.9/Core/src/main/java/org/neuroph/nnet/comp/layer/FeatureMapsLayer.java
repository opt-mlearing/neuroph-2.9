...
/**
 * This class represents an array of feature maps which are 2 dimensional layers(Layer2D instances)
 * and it is base class for Convolution[卷积层] and Pooling[池化层] layers, which are used in ConvolutionalNetwork.
 *
...
public abstract class FeatureMapsLayer extends Layer {
    ...
    /**
     * Among deep learning, may be need multiple featureMapLayer to extract different kinds of feature.
     * so, you need a list of FeatureMapLayer to constitute a list.
     *
     * @return a list of FeatureMapLayer instance.
     */
     ...
     */
    public FeatureMapsLayer(Dimension2D kernelDimension, Dimension2D mapDimensions,
                            int mapCount, NeuronProperties neuronProp) {
        this.mapDimensions = mapDimensions;
        this.featureMaps = new ArrayList<>();
     ...
     */
    protected final void createFeatureMaps(int mapCount, Dimension2D mapDimensions,
                                           Dimension2D kernelDimension, NeuronProperties neuronProperties) {
    ...
    public int getNeuronsCount() {
        int neuronCount = 0;
        for (FeatureMapLayer singleFeatureMapLayer : featureMaps) {
            neuronCount += singleFeatureMapLayer.getNeuronsCount();
        }
        return neuronCount;
    }
